...
public class Logica {
	...
	public Logica(PApplet app) {
		this.app = app;
		minim = new Minim(app);
		cancion = minim.loadFile("dubster.mp3");
		font1 = app.createFont("big_noodle_titling.ttf", 48);
		app.createFont("athletic_regular.ttf", 48);
...
//			}
		...
		case 3:
			app.image(score, 0, 0);
			if (per.getcontadorM()==10 ||per.getcontadorM()>10 ) {
				app.image(ganaste, 532, 249);
				app.image(pgritos, 532, 361);	
				app.textFont(font1);
				app.textSize(30);
				app.fill(226,173,96);					
				app.text(per.getcontadorM() +"x1000 = " + per.getcontadorM()+"000 " +" GRITOS", 635, 397);
				}else {
					app.image(perdiste, 532, 238);	
					app.image(pgritos, 530, 361);	
					app.textFont(font1);
					app.textSize(30);
					app.fill(226,173,96);					
					app.text(per.getcontadorM() +"x1000 = " + per.getcontadorM()+"000 " +" GRITOS", 650, 397);
